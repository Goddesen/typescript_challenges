...
type Chainable<Values extends Record<string, never> = {}> = {
  option<K extends string, V extends any>(key: K, value: V): Chainable<Omit<Values, K> & Record<K, V>>
  get(): Values
}
...
const result2 = a
  ...
  // XXX: This shoudn't be an error according to the specs
  // @ts-expect-error
